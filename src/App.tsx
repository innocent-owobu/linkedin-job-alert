...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 flex flex-col font-sans antialiased">
      ...
      <header id="main_header" className="bg-[#1e293b] border-b border-slate-700/80 px-6 py-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono mt-0.5">Deployment: Vercel Hobby • Germany Region Focus</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {/* UTC Clock Badge */}
          <div className="flex items-center gap-2 bg-slate-800/60 px-3.5 py-1.5 rounded-full border border-slate-700/50 text-xs font-mono text-slate-300">
          ...
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-semibold ${
            config.telegramBotToken && config.telegramChatId
              ...
              : 'bg-slate-700/30 text-slate-400 border-slate-700/20'
          }`}>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden">
          ...
          <div className="bg-[#111827] border border-slate-800/80 rounded-2xl p-6 shadow-xl flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <span className={`px-2.5 py-1 text-[10px] rounded font-mono uppercase font-semibold ${
                status.status === 'idle' ? 'bg-slate-800/60 text-slate-400 border border-slate-700/50' :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-1">
              ...
              <button
                ...
                className="flex flex-col items-start p-4 rounded-xl border border-slate-800 bg-[#0f172a] hover:bg-slate-800/40 hover:border-slate-700 disabled:opacity-40 transition-all text-left group"
              >
                ...
                <span className="text-[10px] text-slate-500 mt-1 font-mono">Trigger Bright Data search</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 mt-1 font-mono">Evaluate & Alert</span>
              </button>

            </div>

            {/* Redis/KV State Display */}
            <div className="bg-slate-800/30 border border-slate-800/80 rounded-xl p-4 text-xs font-mono space-y-2.5">
              <div className="text-indigo-400 font-bold uppercase text-[10px] tracking-wider pb-1.5 border-b border-slate-800/60">
                Upstash Redis Cache Metadata
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Snapshot Pointer ID:</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Checked & Alerted:</span>
                <span className="text-slate-300">
        ...
        <section id="display_column" className="lg:col-span-7 flex flex-col gap-6 bg-[#111827]/90 border border-slate-800/80 rounded-2xl p-6 md:p-8 shadow-xl">
          
          {/* TAB BAR HEADER */}
          <nav id="tabs_navigation" className="flex border-b border-slate-800/80 pb-4 flex-wrap gap-2">
            {[
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-slate-800/30 p-5 rounded-2xl border border-slate-800/80">
                      <p className="text-slate-500 text-xs font-semibold mb-1 uppercase tracking-wider">Alerts (Session)</p>
                      <p className="text-3xl font-bold text-white font-display">{alerts.length}</p>
                    </div>
                    <div className="bg-slate-800/30 p-5 rounded-2xl border border-slate-800/80">
                      <p className="text-slate-500 text-xs font-semibold mb-1 uppercase tracking-wider">Tests Configured</p>
                      <p className="text-3xl font-bold text-indigo-400 font-display">{testCases.length}</p>
                    </div>
                    <div className="bg-slate-800/30 p-5 rounded-2xl border border-slate-800/80">
                      <p className="text-slate-500 text-xs font-semibold mb-1 uppercase tracking-wider">Matched Roles</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-800/30 border border-slate-800/80 p-5 rounded-2xl">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['Data Analyst', 'BI Analyst', 'Tableau Developer', 'Power BI Developer', 'Business Intelligence Analyst', 'Business Analyst', 'Insights and Report Specialist', 'Business Data Analyst'].map((role) => (
                          <span key={role} className="text-[10px] px-2 py-0.5 bg-slate-800 border border-slate-700 rounded text-slate-300 font-mono">
                            {role}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="bg-slate-800/30 border border-slate-800/80 p-5 rounded-2xl">
                      ...
                      <ul className="text-xs space-y-2 text-slate-300">
                        <li className="flex justify-between">
                          <span className="text-slate-400">Max Applicants</span> 
                          <span className="text-white font-mono bg-slate-800 px-2 py-0.5 rounded border border-slate-700">15</span>
                        </li>
                        <li className="flex justify-between">
                          <span className="text-slate-400">Freshness (H)</span> 
                          <span className="text-white font-mono bg-slate-800 px-2 py-0.5 rounded border border-slate-700">2.0</span>
                        </li>
                        <li className="flex justify-between">
                  ...
                  <div className="bg-slate-800/20 border border-slate-800/80 p-5 rounded-2xl space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-center text-xs pt-1">
                      <div className="bg-[#0f172a] border border-slate-800 p-3.5 rounded-xl">
                        ...
                        <div className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">Runs on the hour. Initiates scraper and saves intermediate snapshot ID in Redis.</div>
                      </div>
                      ...
                      <div className="bg-[#0f172a] border border-slate-800 p-3.5 rounded-xl">
                        ...
                        <div className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">Runs at +8m offset. Downloads data, evaluates state engine boundary transitions, alerts via Telegram.</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900/50 border border-slate-800/80 p-4 rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">Ready for automated cron job polling intervals</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-[10px] font-mono bg-[#0f172a] text-slate-400 px-2 py-1 rounded border border-slate-800">v1.2.0-sleek</span>
                      <span className="text-[10px] font-mono bg-[#0f172a] text-slate-400 px-2 py-1 rounded border border-slate-800">React + Vite</span>
                    </div>
                  </div>
                ...
                >
                  <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Validates the parser, deduplication rules, and applicant thresholds.</p>
                    </div>
                    <button
                      id="btn_run_tests"
                  ...
                  <div className="space-y-3 overflow-y-auto max-h-[500px] pr-1">
                    {testCases.map((tc) => {
                      ...
                      return (
                        <div key={tc.id} className="bg-slate-800/30 border border-slate-800 rounded-xl p-4 space-y-3">
                          <div className="flex justify-between items-start gap-4">
                            ...
                            {result ? (
                              ...
                              <span className="text-[10px] text-slate-500 italic">Pending execution</span>
                            )}
                          </div>

                          {/* Details & Specs */}
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px] bg-[#0f172a] p-3 rounded-lg border border-slate-800/60 font-mono text-slate-300">
                            <div>
                              <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Raw Count:</span>
                              <span>{tc.job.applicantCountRaw === null ? 'null' : String(tc.job.applicantCountRaw)}</span>
                            </div>
                            <div>
                              <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Post Age:</span>
                              <span>{tc.job.postedAt}</span>
                            </div>
                            <div>
                              <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Prev Band:</span>
                              <span>{tc.previousBand || 'none'}</span>
                            </div>
                            <div>
                              <span className="text-slate-500 block text-[9px] uppercase tracking-wider font-semibold">Role Fit:</span>
                          ...
                          {result && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs bg-[#0f172a] border border-slate-800/50 p-3.5 rounded-lg text-[11px]">
                              <div className="space-y-1">
                                <div className="text-slate-500 font-bold uppercase text-[9px] tracking-wider">Engine Evaluation:</div>
                                <div className="space-y-1 font-mono text-slate-300">
                                  <div>Current Band: <strong className="text-slate-100">{result.currentBand}</strong> (Expected: <strong className="text-slate-400">{tc.expectedBand}</strong>)</div>
                                  <div>Alert Action: <strong className={result.shouldAlert ? 'text-amber-400' : 'text-slate-400'}>{result.shouldAlert ? 'ALERTED' : 'NO ALERT'}</strong></div>
                                  <div>Alert Type: <span className="text-indigo-400">{result.alertType}</span></div>
                                </div>
                              </div>
                              <div className="p-3 bg-slate-800/40 border border-slate-700/40 rounded text-slate-400 leading-relaxed text-[11px]">
                                <span className="font-semibold text-slate-300 block mb-1">Pass Rule Justification:</span>
                                {tc.explanation}
                              </div>
                            </div>
                          )}
              ...
              {activeTab === 'alerts' && (
                ...
                >
                  <div className="border-b border-slate-800 pb-3">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">Dispatched alerts log copy. Preserves full markdown templates sent to the channel.</p>
                  </div>

                  <div className="space-y-4 overflow-y-auto max-h-[450px] flex-1 pr-1">
                    {alerts.length === 0 ? (
                      <div className="text-slate-500 italic flex items-center justify-center py-20 border border-dashed border-slate-800 rounded-2xl bg-slate-900/10">
                      ...
                      alerts.map((alertItem, idx) => (
                        <div key={idx} className="bg-slate-800/30 border border-slate-800 rounded-xl p-4 flex flex-col gap-3">
                          <div className="flex justify-between items-center border-b border-slate-800/60 pb-2">
                            <div className="flex items-center gap-2">
                              <span className="p-1 bg-indigo-500/10 text-indigo-400 rounded-md">
                                <Send className="w-3.5 h-3.5" />
                              </span>
                              <span className="text-[10px] text-slate-400 font-mono">{alertItem.timestamp}</span>
                            </div>
                          ...
                          <div className="bg-[#0f172a] border border-slate-800 p-4 rounded-lg font-mono text-[11px] text-slate-200 whitespace-pre-wrap leading-relaxed shadow-inner">
                            {alertItem.message}
                          </div>
                        </div>
                      ))
                ...
                >
                  <form onSubmit={handleSaveConfig} className="space-y-6">
                    <div className="border-b border-slate-800 pb-3">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Manage live keys. Values are loaded dynamically from server configurations.</p>
                    </div>
                    ...
                    <div className="p-5 bg-indigo-950/20 border border-indigo-900/30 rounded-2xl flex items-center justify-between gap-6 shadow-sm">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                          config.useSimulatedApis ? 'bg-indigo-600' : 'bg-slate-800'
                        }`}
                      >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                      ...
                      <div className="space-y-3 bg-slate-800/20 border border-slate-800 p-5 rounded-2xl">
                        <h4 className="font-bold text-white border-b border-slate-800 pb-2 mb-2 uppercase tracking-wide text-slate-300">1. Bright Data Settings</h4>
                        <div className="space-y-1">
                          <label className="text-slate-400">Bright Data API Key</label>
                          <input
                            type="password"
                            value={config.brightDataApiKey}
                            onChange={e => setConfig(prev => ({ ...prev, brightDataApiKey: e.target.value }))}
                            placeholder="Dataset API Key"
                            className="w-full bg-slate-950 border border-slate-800 p-2.5 rounded-lg text-slate-100 placeholder-slate-700 focus:border-indigo-500 focus:outline-none font-mono"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-slate-400">Dataset ID (LinkedIn Jobs)</label>
                          <input
                            type="text"
                      ...
                      <div className="space-y-3 bg-slate-800/20 border border-slate-800 p-5 rounded-2xl">
                        <h4 className="font-bold text-white border-b border-slate-800 pb-2 mb-2 uppercase tracking-wide text-slate-300">2. Telegram Bot Settings</h4>
                        <div className="space-y-1">
                          <label className="text-slate-400">Telegram Bot Token</label>
                          <input
                            type="password"
                        ...
                        <div className="space-y-1">
                          <label className="text-slate-400">Telegram Chat / Channel ID</label>
                          <input
                            type="text"
                      ...
                      <div className="space-y-3 bg-slate-800/20 border border-slate-800 p-5 rounded-2xl">
                        <h4 className="font-bold text-white border-b border-slate-800 pb-2 mb-2 uppercase tracking-wide text-slate-300">3. Upstash Redis / Vercel KV</h4>
                        <div className="space-y-1">
                          <label className="text-slate-400">Upstash REST URL</label>
                          <input
                            type="text"
                        ...
                        <div className="space-y-1">
                          <label className="text-slate-400">Upstash REST Token</label>
                          <input
                            type="password"
                      ...
                      {/* SECURITY & TOKEN */}
                      <div className="space-y-3 bg-slate-800/20 border border-slate-800 p-5 rounded-2xl">
                        <h4 className="font-bold text-white border-b border-slate-800 pb-2 mb-2 uppercase tracking-wide text-slate-300">4. Pipeline Access Security</h4>
                        <div className="space-y-1">
                          <label className="text-slate-400">Cron Authorization Secret</label>
                          <input
                            type="text"
              ...
              {activeTab === 'guide' && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -5 }}
                  className="space-y-6 text-xs text-slate-300 leading-relaxed max-h-[500px] overflow-y-auto pr-2"
                >
                  <h3 className="text-base font-bold text-white border-b border-slate-800 pb-2 mb-3 font-display">
                  ...
                  <div className="space-y-2">
                    ...
                    <ol className="pl-14 list-decimal space-y-1 text-slate-400 font-mono text-[11px]">
                  ...
                  <div className="space-y-2">
                    ...
                    <ol className="pl-14 list-decimal space-y-1 text-slate-400 font-mono text-[11px]">
                      <li>Message <strong className="text-slate-200">@BotFather</strong> on Telegram and send <code>/newbot</code>.</li>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pl-8 mt-2">
                      <p className="text-slate-400 font-semibold mb-1">Set these Environment Variables in your Vercel Dashboard:</p>
                      <pre className="bg-[#0f172a] border border-slate-800 p-4 rounded-xl text-[10px] font-mono mt-1 text-slate-300 shadow-inner">
...
SHARED_SECRET="select_a_secure_token_for_cron_job_auth"`}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pl-8 space-y-3 mt-2">
                      <div className="bg-[#0f172a] border border-slate-800 p-4 rounded-xl">
                        ...
                        <ul className="list-disc pl-4 text-slate-400 space-y-1.5 mt-2 font-mono text-[10px]">
      ...
      {/* FOOTER BAR */}
      <footer id="main_footer" className="mt-auto border-t border-slate-800/80 bg-[#111827] px-6 py-4 text-center text-xs text-slate-500 flex flex-col sm:flex-row justify-between items-center gap-2 shadow-inner">
